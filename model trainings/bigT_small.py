...
config = {
    ...
    'model_save_path': 'bigT_model_tam.pth',
...
def initialize_model(num_classes, model_name='resnetv2_101x1_bitm'):
    model = timm.create_model(model_name, pretrained=True)

    if hasattr(model, 'head') and hasattr(model.head, 'fc') and isinstance(model.head.fc, nn.Conv2d):
        in_channels = model.head.fc.in_channels
        model.head.fc = nn.Conv2d(in_channels, num_classes, kernel_size=1, stride=1)
...
def evaluate_model(model,test_loader):
    ...
    df_report.to_csv("bigT.csv", index=True)
